...
class MyStr(str):
    """Класс MyStr, который наследуется от встроенного класса str и добавляет дополнительную информацию"""

    def __new__(cls, value, author, *args, **kwargs):
        obj = super().__new__(cls, value)
        obj.value = value
        obj.author = author
        obj.creation_time = datetime.now().strftime('%Y-%m-%d %H:%M')
        return obj

    def __str__(self):
        return f"{self.value} (Автор: {self.author}, Время создания: {self.creation_time})"
